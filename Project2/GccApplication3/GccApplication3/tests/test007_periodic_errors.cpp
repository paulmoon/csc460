...
extern int r_main(){    
	...
	//uint8_t e = ERR_RUN_8_TWO_PERIODIC_TASKS_READY;
	//uint8_t e = ERR_RUN_9_PERIODIC_TASK_TIME_CONFLICT;
		
    if( e == ERR_RUN_7_PERIODIC_TOOK_TOO_LONG){
        Task_Create_Periodic(loop_forever,0,10,8,0);

    }else if( e == ERR_RUN_5_INVALID_WCET_AND_PERIOD){
        Task_Create_Periodic(p,0,5,10,0);		
		        
    }else if( e == ERR_RUN_6_INVALID_START_TIME){
    
        _delay_ms(50);
        Task_Create_Periodic(p,0,5,1,0);

    }else if ( e == ERR_RUN_8_TWO_PERIODIC_TASKS_READY){
    ...
    }else if ( e == ERR_RUN_9_PERIODIC_TASK_TIME_CONFLICT){
